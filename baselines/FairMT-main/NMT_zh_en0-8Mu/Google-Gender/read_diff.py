...
from transformers import BertConfig, BertTokenizer, BertModel, RobertaTokenizer, RobertaModel, BertForMaskedLM, \
    ElectraForPreTraining, ElectraTokenizerFast, ElectraModel, AutoTokenizer, AutoModel
...
def BertSim(en1, en2, zh1, zh2, berten, bertzh, berttokenizeren, berttokenizerzh):
    ...
    return [zhsim, 0,
            0]  # CosSim(zh1v, en1vmap), CosSim(zh2v, en2vmap)] #sim1 #ensim - zhsim#E3#ensim - zhsim#sim2 #- 0.97#[sim1, sim2, E1, E2]#int(sim1 - 0.85 > 0 or E1 ) - 0.5#[(1 - zhsim) / (1 - ensim), sim1, sim2, E1, E2] # - 0.998#- abs(zhsim - ensim) + pns
...
def read_from_wdiff(old_sentence, new_sentence):
    # print (old_sentence)
    memory1 = os.path.join(base_dir, "memory_1.txt")
    memory2 = os.path.join(base_dir, "memory_2.txt")

    f = open(memory2, "w")
    f.write(old_sentence.strip().replace("[", "!@[#$").replace("]", "!@]#$").replace("{", "!@{#$     ").replace("}",
                                                                                                                "!@}#$"))
    f.close()
    f = open(memory1, "w")
    f.write(
        new_sentence.strip().replace("[", "!@[#$").replace("]", "!@]#$").replace("{", "!@{#$").replace("}", "!@}#$"))
    f.close()

    diff = os.popen(f"wdiff {memory1} {memory2}")
...
def com_bleu(en, new, ora, en_old, en_new):
    ...
    or_lines = []
    en_lines = []
    f = open(os.path.join(base_dir,"ans.bleu"), "w")
    l = []
    for i in range(len(en)):
        ...
        score = compute_bleu(or_lines,
                             en_lines)  # nltk.translate.bleu_score.sentence_bleu([or_line], en_line, weights=(0.25, 0.25, 0.25, 0.25), smoothing_function=nltk.translate.bleu_score.SmoothingFunction.method4)
        score_new = compute_bleu(or_lines,
                                 en_new_lines)  # nltk.translate.bleu_score.sentence_bleu([or_line], en_line, weights=(0.25, 0.25, 0.25, 0.25), smoothing_function=nltk.translate.bleu_score.SmoothingFunction.method4)
        # print (en_line)
        ...
        # print (score)
        sum_ += score
    f_com = open(os.path.join(base_dir,"Com_BLEU.txt"), "w")
    l = sorted(l)[::-1]
...
f = open(os.path.join(base_dir,"Com_Mutated.en"), "r")
en_new_lines = f.readlines()
f.close()

f = open(os.path.join(base_dir,"Com_Original.en"), "r")
en_old_lines = f.readlines()
f.close()

f = open(os.path.join(base_dir,"Com_Mutated.zh"), "r")
new_lines = f.readlines()
f.close()

f = open(os.path.join(base_dir,"Com_Original.zh"), "r")
old_lines = f.readlines()
f.close()

f = open(os.path.join(base_dir,"Com_oracle.zh"), "r")
...
f_edit = open(os.path.join(base_dir,"Com_Answer.zh"), "w")
...
for i in tqdm(range(len(old_lines))):
    old_line = jieba.cut(old_lines[i].strip(), cut_all=False)  # .split()
    new_line = jieba.cut(new_lines[i].strip(), cut_all=False)  # .split()
    ...
    bertsim = BertSim(en_old_lines[i], en_new_lines[i], old_lines[i], new_lines[i], berten, bertzh, tokenizeren,
                      tokenizerzh)
    ...
    lcs_d.append(
        deepcopy([lcs_d_sim, -1, -1, old_lines[i], en_old_lines[i], new_lines[i], en_new_lines[i], ora_lines[i]]))

    tf_d_sim = tf_cos_d_sim(old_tokens, new_tokens)
    tfidf_d.append(
        deepcopy([tf_d_sim, -1, -1, old_lines[i], en_old_lines[i], new_lines[i], en_new_lines[i], ora_lines[i]]))

    delta = abs(old_lcs_sim - new_lcs_sim)
    lcs_l.append(deepcopy(
        [delta, old_lcs_sim, new_lcs_sim, old_lines[i], en_old_lines[i], new_lines[i], en_new_lines[i], ora_lines[i]]))
    ...
    ed_l.append(deepcopy(
        [delta, old_ed_sim, new_ed_sim, old_lines[i], en_old_lines[i], new_lines[i], en_new_lines[i], ora_lines[i]]))
    ed_d.append(
        deepcopy([ed_d_sim, -1, -1, old_lines[i], en_old_lines[i], new_lines[i], en_new_lines[i], ora_lines[i]]))
    bleu_d_sim = bleu_sim_d(old_tokens, new_tokens)

    bleu_d.append(
        deepcopy([bleu_d_sim, -1, -1, old_lines[i], en_old_lines[i], new_lines[i], en_new_lines[i], ora_lines[i]]))
    ...
    tfidf_l.append(deepcopy(
        [delta, old_ti_sim, new_ti_sim, old_lines[i], en_old_lines[i], new_lines[i], en_new_lines[i], ora_lines[i]]))
...
def write_all(lcs, ed, tfidf, bleu, lcs_d, ed_d, tfidf_d, bert, file_name):
    ...
    for site in range(len(l)):
        ...
        if tfidf[site][0] > 0.1 * max(tfidf[site][1], tfidf[site][2]):
            flag = True
        flag = str(flag)
        f.write("Tf-idf:\t" + str(tfidf[site][0]) + " " + str(tfidf[site][1]) + " " + str(
            tfidf[site][2]) + " " + flag + "\n")
        flag = False
        ...
        flag = str(flag)
        f.write(
            "BLEU:\t" + str(bleu[site][0]) + " " + str(bleu[site][1]) + " " + str(bleu[site][2]) + " " + flag + "\n")
    ...
    divisor = len(l) if len(l) != 0 else 1
    for i in range(4):
        sum_list[i] = sum_list[i] / divisor if len(l) != 0 else 0   
    f.close()


write_all(lcs_l, ed_l, tfidf_l, bleu_l, lcs_d, ed_d, tfidf_d, bert, os.path.join(base_dir,"Com_ALL.txt"))
...
bert = sorted(bert)[::-1]
write_file(bert, os.path.join(base_dir,"Com_BERT.txt"))
