...
#    ensim = util.dot_score(en1v, en2v)
    ...
    return [zhsim, 0, 0]#CosSim(zh1v, en1vmap), CosSim(zh2v, en2vmap)] #sim1 #ensim - zhsim#E3#ensim - zhsim#sim2 #- 0.97#[sim1, sim2, E1, E2]#int(sim1 - 0.85 > 0 or E1 ) - 0.5#[(1 - zhsim) / (1 - ensim), sim1, sim2, E1, E2] # - 0.998#- abs(zhsim - ensim) + pns
...
def read_from_wdiff(old_sentence, new_sentence):
    ...
    f.close()
    f = open(memory1, "w")
    f.write(new_sentence.strip().replace("[", "!@[#$").replace("]", "!@]#$").replace("{", "!@{#$").replace("}", "!@}#$"))
    f.close()
