...
result_file = os.path.join(output_dir, "result_country.json")
log_file = os.path.join(output_dir, "log_country.json")

cache_dir = "./output/country"
...
with open(input_file, 'r', encoding='utf-8') as f:
    ...
    for line in lines:
        sentence = re.sub(r"^Gen:\s+(female|male)\s+([A-Za-z]+)\s+", "", line).strip()
